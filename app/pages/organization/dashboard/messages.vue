...
<template>
  <div class="flex-1 flex overflow-hidden min-h-0">
    ...
    <aside class="w-64 sm:w-72 md:w-80 flex flex-col bg-background border-r border-border/50 shrink-0 min-w-0">

      <!-- Header -->
      <div class="flex items-center justify-between px-4 py-4 border-b border-border/50 shrink-0">
      ...
      <div class="flex-1 overflow-y-auto">
        <button
          ...
          :key="conv.id"
          :class="[
            'w-full flex items-center gap-3 px-4 py-3.5 text-left transition-colors border-b border-border/30',
            selectedId === conv.id ? 'bg-primary/10 dark:bg-primary/20' : 'hover:bg-accent',
          ]"
        ...
        >
          ...
          <div class="relative shrink-0">
            ...
            <span :class="['absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-background', conv.online ? 'bg-green-500' : 'bg-muted-foreground/50']" />
          </div>
        ...
        <div v-if="filteredConversations.length === 0" class="py-12 text-center">
          <Search class="w-8 h-8 text-muted-foreground/50 mx-auto mb-2" />
    ...
    <main class="flex-1 flex flex-col min-w-0 bg-background">
      ...
      <!-- Chat header -->
      <header class="shrink-0 flex items-center justify-between px-5 py-3.5 border-b border-border/50 bg-background">
        <div class="flex items-center gap-3">
          <div class="relative">
            ...
            <span :class="['absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2 border-background', selected.online ? 'bg-green-500' : 'bg-muted-foreground/50']" />
          </div>
          <div>
            <p class="text-sm font-semibold text-foreground leading-tight">{{ selected.name }}</p>
            <p :class="['text-xs leading-tight', selected.online ? 'text-green-600' : 'text-muted-foreground']">
              ...
              <div :class="['flex flex-col gap-1 max-w-[70%]', msg.from === 'me' ? 'items-end' : 'items-start']">
                ...
                <div class="flex items-center gap-1">
                  ...
                  <template v-if="msg.from === 'me'">
                    ...
                    <Check v-else class="w-3.5 h-3.5 text-muted-foreground/70" />
                  </template>
                </div>
              </div>
            </div>
      ...
      <!-- Compose bar -->
      <footer class="shrink-0 px-5 py-3.5 border-t border-border/50 bg-background">
        <div class="flex items-center gap-3 bg-muted/50 border border-border rounded-2xl px-4 py-2.5 focus-within:ring-2 focus-within:ring-primary focus-within:border-transparent transition-all">
          ...
          <button
            :class="['w-8 h-8 rounded-xl flex items-center justify-center transition-colors shrink-0', compose.trim() ? 'bg-primary text-primary-foreground hover:bg-primary/90' : 'bg-muted text-muted-foreground cursor-default']"
