    ...
    <div class="relative z-10 w-full max-w-[372px]">
      ...
      >
        ...
        <div class="p-8 pt-7">
          ...
          <button
            :disabled="loading"
            class="w-full flex items-center justify-center gap-3 transition-all duration-150 active:scale-[0.983] disabled:opacity-60 disabled:cursor-not-allowed"
            style="height: 46px; border-radius: 12px; border: 1.5px solid oklch(0.9 0 0); background: white; font-size: 13.5px; font-weight: 500; color: oklch(0.2 0 0); box-shadow: 0 1px 4px rgba(0,0,0,0.06);"
            @mouseenter="e => !loading && (e.currentTarget.style.background = 'oklch(0.985 0 0)')"
            @mouseleave="e => (e.currentTarget.style.background = 'white')"
            @click="continueWithGoogle"
          >
            <!-- Google SVG -->
            <svg v-if="!loading" width="18" height="18" viewBox="0 0 24 24" class="flex-shrink-0">
