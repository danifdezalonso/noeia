...
import { Search, Video, MapPin, Link2, Copy, Check, X } from 'lucide-vue-next'
  ...
  <Dialog :open="open" @update:open="(v) => !v && handleClose()">
    <DialogContent class="flex flex-col gap-0 p-0 max-h-[85vh] sm:max-w-lg">
      ...
      <div class="flex-1 overflow-y-auto px-6 py-5 space-y-5 min-h-0">
        ...
        <!-- ── Client ──────────────────────────────────────────────────── -->
        <div>
          <Label class="block text-xs font-medium text-muted-foreground mb-1.5">Client <span class="text-red-500">*</span></Label>
          <div ref="patientWrapRef" class="relative">
            <div class="relative">
              ...
              />
              <button v-if="form.patientId" class="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground/70 hover:text-muted-foreground transition-colors" @click="clearPatient">
                <X class="w-3.5 h-3.5" />
              </button>
            </div>
            ...
            <Transition enter-active-class="transition duration-100 ease-out" enter-from-class="opacity-0 translate-y-1" enter-to-class="opacity-100 translate-y-0">
              <div v-if="showPatients && patientList.length > 0" class="absolute top-full left-0 right-0 mt-1.5 bg-card border border-border rounded-xl shadow-xl z-10 py-1 max-h-44 overflow-y-auto">
                <button
                  v-for="p in patientList"
                  :key="p.id"
                  class="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-left hover:bg-accent transition-colors"
        ...
        <!-- ── Type ────────────────────────────────────────────────────── -->
        <div>
          <Label class="block text-xs font-medium text-muted-foreground mb-1.5">Session type</Label>
        ...
        <!-- ── Modality toggle ─────────────────────────────────────────── -->
        <div>
          <Label class="block text-xs font-medium text-muted-foreground mb-1.5">Format</Label>
          <div class="flex rounded-xl border border-border overflow-hidden">
            <button
              :class="['flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-medium transition-colors', form.modality === 'online' ? 'bg-primary text-primary-foreground' : 'bg-card text-muted-foreground hover:bg-accent']"
              @click="form.modality = 'online'"
            >
              <Video class="w-3.5 h-3.5" /> Online
            </button>
            <button
              :class="['flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-medium border-l border-border transition-colors', form.modality === 'inperson' ? 'bg-primary text-primary-foreground border-primary' : 'bg-card text-muted-foreground hover:bg-accent']"
          ...
          <div v-if="form.modality === 'online'" class="mt-2.5">
            <div class="flex gap-2 items-center">
              <div class="relative flex-1">
                <Link2 class="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-muted-foreground pointer-events-none z-10" />
                <Input
                  v-model="form.meetingLink"
                  type="url"
                  class="pl-9 text-xs bg-muted/50 text-muted-foreground"
                />
              </div>
          ...
          <div v-else class="mt-2.5 space-y-2">
            <div class="flex flex-wrap gap-1.5">
              <button
                ...
                :key="loc"
                :class="['px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors', !form.locationIsCustom && form.location === loc ? 'bg-primary text-primary-foreground border-primary' : 'bg-card border-border text-muted-foreground hover:bg-accent']"
                @click="form.location = loc; form.locationIsCustom = false"
              >
                {{ loc }}
              </button>
              <button
                :class="['px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors', form.locationIsCustom ? 'bg-primary text-primary-foreground border-primary' : 'bg-card border-border text-muted-foreground hover:bg-accent']"
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <Label class="block text-xs font-medium text-muted-foreground mb-1.5">Date <span class="text-red-500">*</span></Label>
            <Input
              v-model="form.date"
              type="date"
          ...
          <div>
            <Label class="block text-xs font-medium text-muted-foreground mb-1.5">Start time <span class="text-red-500">*</span></Label>
            <Input
              v-model="form.time"
              type="time"
        ...
        <!-- ── Duration ────────────────────────────────────────────────── -->
        <div>
          <Label class="block text-xs font-medium text-muted-foreground mb-1.5">Duration</Label>
          <div class="flex gap-1.5 flex-wrap">
            <button
              v-for="d in DURATIONS"
              :key="d.value"
              :class="['px-3.5 py-2 text-sm font-medium rounded-xl border transition-colors', form.duration === d.value ? 'bg-primary text-primary-foreground border-primary' : 'bg-card border-border text-muted-foreground hover:bg-accent']"
              @click="form.duration = d.value"
            >
              {{ d.label }}
            </button>
          </div>
          <p v-if="form.time && form.date" class="mt-1.5 text-xs text-muted-foreground">
